...
| 8C | Days-rest model | **MIXED — `max_reduction` 0.10→0.07 RESOLVED (Group 1, 3836cb2); naming mislabel → `DAYS_REST_EFOLD_TIME` RESOLVED (Group 1, 3836cb2); travel/altitude/density omissions DATA_GATED (Plan 10 §Group C — wrong metric, no code change)**; decay form CONFIRMED; role gradient + channel CONFIRMED_WITH_CAVEAT |
...
| 8G | Injury redistribution model | MIXED — structure + MIN_ELIGIBLE CONFIRMED; PRIMARY_SHARE/EFFICIENCY ACCEPTABLE/DATA_GATED; **usage-concentration DATA_GATED (Plan 10 §Group D — metric re-specified, no code change)** |
...
| Travel / altitude / density omissions | **DATA_GATED (Plan 10 §Group C)** (material — wrong metric) | Westward ~9pp swing; Denver altitude largest HCA in sports; 4-in-5 ≈ −1 pt/100 — each rivals the modeled B2B effect. Plan 10: the proposed minutes-haircut terms were the **wrong metric** — altitude should be a per-possession efficiency multiplier (~0.5–1.0%), westward a win-prob/efficiency tilt keyed to time-zones crossed × game time (its circadian sign is contested by a 2024 25,016-game study); DATA_GATED on DEN/UTA + travel-direction backtests, no code change |
...
| Usage-concentration omission | **DATA_GATED (Plan 10 §Group D)** | Usage concentrates on the next-best creator, not evenly on minute-fillers; flat minutes-based redistribution under-projects the secondary creator (esp. AST/PTS) — highest-value improvement. Plan 10: the creator threshold must be **AST%** (share of a teammate's FGs assisted while on court), **not** "20% of team AST" (under-discriminates — every player in a 5-man rotation trivially averages ~20%); the proposed `_CREATOR_USAGE_SHARE`=0.30 has **no empirical backing** and is not set in code; DATA_GATED at n≥50 injury-replacement events, no code change |
...
| 2 | **Usage-concentration omission** ◐ DATA_GATED (Plan 10 §Group D) | 8G | Injury redistribution routes the OUT player's *minutes* by position but spreads *usage* pro-rata; real usage concentrates on the next-best creator (Durant-out→Westbrook) → under-projects the secondary creator's AST/PTS, over-spreads to minute-fillers | Creator threshold re-specified as **AST%** (share of a teammate's FGs assisted while on court), **not** "20% of team AST" (under-discriminates — every player in a 5-man rotation trivially averages ~20%). Proposed `_CREATOR_USAGE_SHARE`=0.30 has **no empirical backing** and is not set in code; the plan_8 reallocation layer was **not implemented** | **DATA_GATED at n≥50 injury-replacement events** (regress actual usage absorbed vs creator tier) before adding any reallocation layer. No code change shipped |
...
| 4 | **Days-rest omissions: travel / altitude / density** ◐ DATA_GATED (Plan 10 §Group C) | 8C | Westward travel ~9pp win swing (Roy & Forest); Denver altitude largest HCA in sports (~67%); 4-in-5 ≈ −1 pt/100 — each rivals the modeled 10% B2B effect; all omitted | Both proposed terms were the **wrong metric**: altitude (proposed `_ALTITUDE_REDUCTION`=0.035) is a minutes haircut → re-specify as a per-possession **efficiency multiplier** (~0.5–1.0%); westward (proposed `_WESTWARD_TRAVEL_REDUCTION`=0.025) is wrong metric **+ contested premise** (2024 25,016-game study finds the circadian sign contested) → re-specify as a **win-prob/efficiency tilt keyed to time-zones crossed × game time**. Neither implemented | **DATA_GATED** on DEN/UTA road backtest (altitude) + travel-direction backtest (westward). No code change shipped |
...
**Group 1 shipped 2026-06-07 (EdgeModel 3836cb2):** #1 (`_REB_RATE_PRIOR` true per-36, all positions — closes H01 BROADER), #3 (`max_reduction` 0.10→0.07, DATA_GATED recal at n≥500 B2B player-games), #5 (`DAYS_REST_HALF_LIFE` → `DAYS_REST_EFOLD_TIME`). **Plan 10 (2026-06-08) resolved #2 and #4 to DATA_GATED with no code change** — #2 (usage-concentration, §8G): creator threshold re-specified as AST% (share of teammate FGs assisted while on court, not "20% of team AST"), proposed `_CREATOR_USAGE_SHARE`=0.30 has no empirical backing, gate n≥50 injury-replacement events; #4 (travel/altitude/density, §8C): both proposed terms were the wrong metric (altitude → per-possession efficiency multiplier; westward → win-prob tilt, contested circadian premise). The plan_8 Group 2/3 code was never implemented. #6 (SF playoff scalar) remains the only open data-gated item. Original audit was research-only; the Group 1 trio were the high-confidence constant fixes.
