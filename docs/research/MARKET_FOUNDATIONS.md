...
**§9F-bis — BM shrinkage ANCHOR decision (Plan 10 P2.3, documented 2026-06-17).** The shrinkage target `implied_p` is the **VIGGED** single-side quote (`implied_prob(odds)`), not no-vig. This is the only shrinkage path (props only); game-line edges are computed directly and never shrunk. EDGE is measured against the **no-vig** prob in every path (props via `calc_edge`→`no_vig`; game lines via explicit `no_vig()`). So anchor=vigged, edge-basis=no-vig — deliberately different and consistently applied (the P2.3 "pick one, align everywhere" ask is satisfied in code). **Known consequence (DATA_GATED reconsideration):** anchoring on vigged while measuring edge vs no-vig means a model exactly at the FAIR (no-vig) price reports a residual edge `= (1−w)·(half-vig)`, *larger* for low-w tiers (T3 ~0.71pp at −110, T2 ~0.36pp; more on juicy markets). Baker–McHale theory anchors on the market's unbiased p_true (= no-vig), so no-vig is the theoretically-correct anchor; vigged retained for now (small magnitude; CLV immature; switching needs `nv_prob` threaded into `apply_bm_shrinkage`). Revisit with the **§B BM-direction-inverted** item at n≥150/family CLV maturity.
