...
3. **Phase 2 quick-wins** — P2.6 ρ-matrix positive-definite check (IN PROGRESS), P2.1 remove STALE files, P2.5 reliability-curve smoke test, P2.10 health_check pre-run gate. Then the rest of Phase 2 (12 tasks) + Phase 3 (incl. n≥150 BM/edge-floor bootstrap refit) per `JonnyParlay_Fix_Plan_v2.md`.
   - *Adaptation notes:* P2.6 → ρ lives in `_pairwise_rho`/`_build_corr_matrix` (sgp_builder + mlb_sgp_builder), NOT `engine/sgp_correlation.py` (doesn't exist). P2.7 Platt-freshness → no Platt JSON artifact (code constant). P2.10 → health_check has 2 standing non-fatal fails (CLAUDE.md>40k, git-not-clean) → must classify blocking vs advisory before gating run_picks on it.
