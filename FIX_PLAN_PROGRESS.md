...
1. **P2.11a** — verify `count_calibration_days()` counts distinct calendar days (gates the highest-leverage Platt refit; day-boundary off-by-one is a recurring bug class — `6bef685`). Confirm what `log_picks` stamps into `date` for calibration rows; add boundary tests (two dates→2, same-day-twice→1, midnight straddle). Fix only if a real bug surfaces.
2. **Phase 3** in order: NB_R consumer regression tests → vig/no-vig property tests → versioned Platt artifact dir → pre-commit hook → remaining hygiene.
