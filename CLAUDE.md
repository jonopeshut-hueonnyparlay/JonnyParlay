...
- `NB_R` (run_picks.py): `3PM`=9.15 (var/mu=1.1486); `AST`=12.16 (var/mu=1.3234, game-level refit 2026-05-30); `REB`=14.7 (var/mu=1.3873, game-level refit 2026-05-30); `HA`=13.41 (var/mu=1.204); `RBI`=0.87 (var/mu=1.535, ~74% zero-RBI games); `ER`=2.62 (var/mu=1.700, bullpen/run-support tails); `HRR`=1.5 (moment-matched shadow log). STL/BLK/TOV Poisson confirmed (var/mu=1.072/1.113/1.050).
- `NB_R_WNBA` (run_picks.py): `AST`=11.37, `REB`=10.74, `3PM`=1.342. AST/REB calibrated 2026-06-04 (202 players / 13,322 game-logs, min≥8). 3PM recalibrated 2026-06-09 (13,970 rows, var/mu=1.708, zero_rate=0.502 — heavily overdispersed vs NBA r=9.15, ~50% zero games).
...
- **WNBA gates** (run_picks.py, Plan 6 §14 rework 2026-06-05): `WNBA_EV_FLOOR`=0.0955 — G_WNBA_EDGE is an EV-per-unit floor (`win_prob/implied_prob(odds) − 1 ≥ 0.0955`, NBA's G9 net EV at −110), computed from quoted odds so it auto-adjusts to vig (replaced dead `WNBA_EDGE_FLOOR`=0.035). Early-season dampener is **sigma inflation**: `WNBA_EARLY_SEASON_EDGE_MULT` factors (days 1–14: 0.80 → σ×1.25; days 15–21: 0.90 → σ×1.11) divide sigma in calc_prop_prob/_combo_mu_sigma/G14 (NB-routed stats shrink prob toward ½) so win_prob, edge, score AND Kelly size shrink coherently. Factors DATA_GATED: recalibration deferred to 2027 season start (go-live fell on day 28 — no usable day-1–21 data). Opening gate keyed to **games played**: `WNBA_OPENING_GATE_GAMES`=2 (both teams, via `_wnba_team_games_played()` on wnba_player_game_stats; `WNBA_OPENING_GATE_DAYS`=3 fallback; evaluated season days 1–14 only). `WNBA_SEASON_START`=2026-05-13 (update each season). `WNBA_TEAM_ABBREV` name→abbrev map (15 teams incl. GSV/TOR/PDX).
- `SIGMA` (run_picks.py): `PTS`=mult 0.35/min 5.0; `REB`=mult 0.48/min 2.0 (combo path only); `AST`=mult 0.53/min 2.0 (combo path only); `OUTS`=mult 0.27/min 1.0; `PC`=mult 0.19/min 6.0 (both recalibrated 2026-06-05, Plan 6 §1C, **starts-only** — prior 0.311/0.375 were contaminated by relief appearances; PC skew −1.93 — Normal provisional, empirical-CDF candidate at July refit); `SV`=mult 0.253/min 3.5 (NHL goalie saves, calibrated 2026-05-26). `HA` removed from SIGMA — now NB_STATS (r=13.41). Note: `MIN_LEG_WIN_PROB_OUTS=0.62` tuned to old σ=0.311; narrower sigma raises OUTS win_probs — monitor SGP leg counts before retuning.
- `GAME_SIGMA["WNBA"]` (run_picks.py): `total`=17.424, `team`=11.253 (spread/ml=10.0 — no active WNBA spread/ML picks). Recalibrated 2026-06-09 from 851 games (priors were placeholders).
...
## WNBA Status
**LIVE as of 2026-06-09.** Posts to Discord, logs to `pick_log.csv`. `SHADOW_SPORTS` empty in run_picks.py/grade_picks.py/post_nrfi_bonus.py. CLV daemon watches main log + legacy `pick_log_wnba.csv` until legacy rows close (then flip `ENABLE_WNBA_CLV=False` in capture_clv.py). Still in force: `SPORT_UNIT_CAP`=4.0u, G_WNBA_EDGE (EV≥0.0955), G_WNBA_OPEN, early-season sigma dampener (2027). **Excluded from KILLSHOT** (sport check in `_passes_killshot_v2_gate`) until CLV history matures. REB pinned to 0.25u floor (mult 0.10 both dirs, revisit n≥50); REB over remains R4-shadow-routed.
