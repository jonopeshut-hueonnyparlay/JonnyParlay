...
- **PICK_SCORE_TIER_MULT T1=0.90×**: Re-evaluate at n=30 T1 picks post-2026-05-23 gates (G8B/G8C/G8D) — **1/30** as of 2026-06-03. Raise to 0.95× if post-gate T1 WR ≥ 55%; remove T1 reserved slots if WR < 50%. **Plan 9 §9F: NEEDS_CHANGE — retire the 0.90× mult AND the n=30 checkpoint (n=30 uninformative, SE ±9.1pp); replace with per-family probability shrinkage (Baker–McHale), merged with the Kelly multiplier-stack consolidation item; formal retirement record = ROI bootstrap at n≥150/family (retire family if P(ROI≥0)<0.10).**
...
- **Kelly multiplier-stack consolidation (DATA_GATED, n≥50 graded per market)**: KELLY_MARKET_MULT × var_m × tier_m chain triple-counts stat-level information. Acceptable today because posted stakes are far below full Kelly and the 0.50u floor quantizes the effect. Future: consolidate to a single empirical-Bayes per-market multiplier (James–Stein at ~50 graded/market); flag any KELLY_MARKET_MULT < 0.30 as cosmetic (0.50u floor overrides it) and consider gating exclusion instead of multiplying (Plan 6 §4).
...
- **Plan 7 EdgeModel NEEDS_CHANGE items (2026-06-06, all July-refit gated)** — full detail in `docs/research/EDGEMODEL_FOUNDATIONS.md` NEEDS_CHANGE table: (1) STL/BLK EWMA span 8→grid {12,15,20,25} at ≥1,500-row backtest; (2) `_AST_EWMA_SPAN` delete-or-invert (uniform 13 simplest) — ordering statistically backwards, PG cell is dead code; (3) AST → Vegas `_base_pf` branch, ship if AST-bias quintile spread >0.15 vs Vegas-divergence; (4) spot=1.6124 root fix — refit with <10-min games excluded from spot EWMA, replace scalar if filtered fit <1.30; (5) PAD_FG≈103 test-gated grid (deploy only on PTS MAE gain); (6) HIGH_VAR comment relabel (cosmetic); (7) align evaluate_projector.py frame with production — the −0.26 3PM "shared bias" is ~⅔ frame artifact, real bias ≈ −0.08. Also: C01 BLK=1.152 confirmed DATA_GATED (league per-possession ≈ flat; expect ~1.05 after Wembanyama LOPO); blend alphas LOCKED — do NOT spend offseason refits on them.
...
- **NRFI elasticity + ERA/FIP blend (Plan 9 §9A, July refit)**: Poisson elasticity `exp(−0.32·m)` is ~50–60% too steep under NB overdispersion (±2pp at pick-firing extremes) — fix via NB zero-prob `(r/(r+μ·m))^r` or exponent dampener `m^γ` (γ≈0.6–0.7); validate on in-house 8,095-game DB first (bucket by predicted multiplier vs realized NRFI rate). ERA/FIP blend 0.40/0.60 → ≤0.25 ERA / ≥0.75 FIP (prefer xFIP/FIP− — also fixes pitcher-side park bias; r²: ERA 0.019 / FIP 0.038 / xFIP 0.061).
- **SGP joint-EV existence floor (Plan 9 §9H)**: per-leg WP floors alone can construct −EV slips on the 4-leg path (the +200–450 window forces per-leg implied 0.653–0.760, above the 0.65/0.62 floors). Add: copula joint_prob > implied(parlay odds) + 0.02–0.03 for ANY slip to fire. Premium gate (margin ≥0.10) unchanged.
...
- **SB26-131 ops note (CO, effective 2026-08-12)**: credit-card deposit ban + max 6 deposits/24h (consumer-protection only — does NOT restrict book limiting). Adjust bankroll funding workflow before Aug 12 2026. Not a code change (Plan 9 §9D).
...
| `docs/research/MARKET_FOUNDATIONS.md` | **Market-facing foundations audit (Plan 9, 2026-06-06).** 11 sections (9A–9K) validating NRFI/YRFI model, anti-correlation filters (X1, ρ bands), CLV methodology, SLOW_BOOKS, parlay construction (Daily Lay/Longshot/SGP), tier system, hard card rules (R4/R7/R9/R10/R12), daily caps. Counts: 24 LOCKED · 9 PERIODIC_RECAL · 13 DATA_GATED · 12 NEEDS_CHANGE (none shipped — all await explicit decision). **Before changing any market-facing constant, gate, or card rule, check this doc.** Headline NEEDS_CHANGE: NRFI elasticity + ERA/FIP blend, SGP joint-EV floor, T1 mult retirement, 0.50u stake floor over-stakes weakest picks 2–2.5×. |
