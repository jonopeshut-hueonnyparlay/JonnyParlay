...
- `SIGMA_WNBA` (run_picks.py): `PTS`=mult 0.618/min 3.5; `AST`=mult 0.779/min 1.0; `REB`=mult 0.633/min 1.0; `3PM`=mult 0.48/min 0.70 (z-score/combo path only — 3PM props now use NB path). Refit 2026-06-04 from 202 players / 13,322 games (2023–2026 WNBA RS). Used for z-score proxy and combo sigma calculations.
...
- `FG3M_BLEND_ALPHA`=0.60 (EdgeModel `engine/nba_projector.py` line 408; `evaluate_projector.py` line 214 default). Grid-search calibrated 2026-06-05, n=2000, 2025-26 season. Curve flat 0.25–0.65 (MAE range 0.004), RMSE-optimal at 0.60. Persistent bias −0.26 (structural, not addressable via alpha).
