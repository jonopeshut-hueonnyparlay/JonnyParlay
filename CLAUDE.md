...
- `PLAYOFF_RATE_DEFLATORS`: pts=0.934, ast=0.845, fg3m=0.948, blk=1.152. Refit 2026-05-10 from 20-date playoff backtest (1071 player-games). BLK is an inflator (under-projected, t=−2.74). Post-fix biases all ≈0; BLK update pending C01 gate (per-possession analysis).
- `PLATT_A`=1.4988, `PLATT_B`=−0.8102 (`calibrated.py`) — **frozen** until H3 gate. Formula: `sigmoid(A * over_p + B)` (**raw-probability space — NOT logit-space**). `PLATT_SPACE="raw"` safeguard constant in `thresholds.py` (consumed by `prob_core.py`) asserts formula space matches; change to `"logit"` simultaneously with formula+A/B at H3. At H3, ALL THREE change simultaneously from calibrate_platt.py output (A/B in `calibrated.py`, PLATT_SPACE in `thresholds.py`).
...
- `SIGMA_WNBA` (`calibrated.py`): `PTS`=mult 0.48/min 3.5; `AST`=mult 0.65/min 1.0; `REB`=mult 0.54/min 1.0; `3PM`=mult 0.48/min 0.70 (z-score/combo path only — 3PM props use NB path). Recalibrated 2026-06-05 on priced population (min≥20, 153 players) — prior min≥8 values were a sampling artifact. Used for G14 z-score proxy and combo sigma calculations.
- **WNBA gates** (`thresholds.py` constants + `wnba_gate.py` logic, Plan 6 §14 rework 2026-06-05): `WNBA_EV_FLOOR`=0.0955 — G_WNBA_EDGE is an EV-per-unit floor (`win_prob/implied_prob(odds) − 1 ≥ 0.0955`, NBA's G9 net EV at −110), computed from quoted odds so it auto-adjusts to vig (replaced dead `WNBA_EDGE_FLOOR`=0.035). Early-season dampener is **sigma inflation**: `WNBA_EARLY_SEASON_EDGE_MULT` factors (days 1–14: 0.80 → σ×1.25; days 15–21: 0.90 → σ×1.11) divide sigma in calc_prop_prob/_combo_mu_sigma/G14 (NB-routed stats shrink prob toward ½) so win_prob, edge, score AND Kelly size shrink coherently. Factors DATA_GATED: recalibration deferred to 2027 season start (go-live fell on day 28 — no usable day-1–21 data). Opening gate keyed to **games played**: `WNBA_OPENING_GATE_GAMES`=2 (both teams, via `_wnba_team_games_played()` on wnba_player_game_stats; `WNBA_OPENING_GATE_DAYS`=3 fallback; evaluated season days 1–14 only). `WNBA_SEASON_START`=2026-05-13 (update each season). `WNBA_TEAM_ABBREV` name→abbrev map (15 teams incl. GSV/TOR/PDX).
- `SIGMA` (`calibrated.py`): `PTS`=mult 0.35/min 5.0; `REB`=mult 0.48/min 2.0 (combo path only); `AST`=mult 0.53/min 2.0 (combo path only); `OUTS`=mult 0.27/min 1.0; `PC`=mult 0.19/min 6.0 (both recalibrated 2026-06-05, Plan 6 §1C, **starts-only** — prior 0.311/0.375 were contaminated by relief appearances; PC skew −1.93 — Normal provisional, empirical-CDF candidate at July refit); `SV`=mult 0.253/min 3.5 (NHL goalie saves, calibrated 2026-05-26). `HA` removed from SIGMA — now NB_STATS (r=13.41). Note: `MIN_LEG_WIN_PROB_OUTS=0.62` tuned to old σ=0.311; narrower sigma raises OUTS win_probs — monitor SGP leg counts before retuning.
...
- `MLB_TEAM_RUN_R`=3.548 (`calibrated.py`): NB dispersion for MLB team run-scoring. Calibrated 2026-06-05 from 8095 regular-season games (var/mu=2.261). Used for team-total NB CDF and ML NB direct sum (`mlb_ml_from_nb()`). MLB HITS/BB/RUNS remain Poisson — within-player var/mu all near 1.0 (0.89/0.97/0.97); no overdispersion.
...
- `MLB_PARK_FACTORS` dict in `calibrated.py` (keyed by home team abbrev, source: Baseball Savant 2022-2025). Not currently applied to projections (park effects already embedded in SaberSim inputs). Available for future park-neutral input paths.
- `KELLY_FRACTION`=6.0 (`thresholds.py`) — continuous Kelly sizing, replaces VAKE step function. Shipped commit 50bf05e. **Label corrected 2026-06-05 (Plan 6 §4): it is a units converter under the 100u bankroll convention (stake fraction = f*×6/100 ≈ 1/16.7 Kelly), NOT "1/6 Kelly". Value unchanged.**
...
- **Team-specific sigmas** (loaded by `_load_team_sigmas()` in `calibrated.py`; lookups in `team_resolve.py`, calibrated 2026-06-05): Per-team scoring distributions for matchup-specific sigma lookups. JSON files: `data/team_sigmas_{nhl,mlb,nba,wnba}.json`. Functions (`team_resolve.py`): `get_game_sigma(sport, home, away, market)` → **relative-scaler formula (Plan 6 §6 rewrite): `σ_league(market) × sqrt((σ_h²+σ_a²)/(2·σ̄²_league))`, n_games≥20 on both teams, else GAME_SIGMA fallback** (old independence sum dropped home/away covariance — inflated NBA spread/ML σ ~45%); `get_game_sigma_team(sport, team)` → team σ or GAME_SIGMA["team"] fallback; `get_mlb_team_run_r(abbr)` → per-team NB r or MLB_TEAM_RUN_R fallback. The 4 main game-line sigma lookups (total/spread/ml/team) use these; F5 lines stay league-wide. Recalibrate with: `python engine/calibrate_distributions.py --mode team-sigmas --sport all`. Mirrored in analyze_game_lines.py. WNBA JSON keys are numeric team_ids — lookup always falls back to GAME_SIGMA["WNBA"] until a WNBA_ID_MAP is added.

## Data-gated / Open
*Current gate counts: see `memory/project_backlog.md`. Quick snapshot: run `python engine/gate_check.py`.*
- **H3 (Platt refit)**: gated on 100 post-v4 graded `over_p_raw` rows — **92/100** as of 2026-06-12 (`python engine/gate_check.py`). **At gate: fit intercept-only (A=1 forced, logit-space) — slope unidentified at n=76–100; free 2-param fit at n≥300.** calibrate_platt.py is in engine/ (restored 2026-06-05, Plan 6). Run: `python engine/calibrate_platt.py --intercept-only --force` (native-only default; `--all-rows` re-includes legacy); deploy only if OOS Brier improves, changing formula + A/B (`calibrated.py`) + PLATT_SPACE (`thresholds.py`) simultaneously.
- **Combo Platt refit gate**: need 100 scored combo picks (RA/PRA/PR/PA) — **23/100** as of 2026-06-12. Check: `python analyze_picks.py --stat RA` (and PRA/PR/PA). Win_prob currently inflated ~5pp vs individual stats — no Platt applied (combo path in `evaluate_props`, `evaluators.py`). Refit only after gate reached and with combo-only Brier improvement check.
- **MLB Platt refit gate**: need 100 MLB `over_p_raw` rows — **27/100** as of 2026-06-12. Check: count non-empty `over_p_raw` in pick_log.csv where sport=MLB. Exclusion documented in `evaluators.py` (evaluate_props). Until gate: MLB win_probs are uncalibrated (unknown inflation direction).
- **Shadow CLV go-live**: need ~100 CLV rows in `pick_log_custom.csv` — **0/100** as of 2026-06-12 (log reset: 227 pre-Plan-6–10 rows from the old miscalibrated model archived to `pick_log_custom_archive_pre_plan10.csv`; gate_check clv==0 over-count fixed in b84a208). **Plan 9 §9C: at gate also require one-sided t≥1.7 on post-reform rows (~150–200 rows realistically); never pool pre/post-reform CLV rows.**
- **Plan 10 fixes shipped** (commits 1dfb31f/13fb3f3/f084010, 2026-06-07): tier moves (RBI/ER→T1, RUNS→T1B, GA/SV→T3, HA→T1, REC→T2); MLB GAME_SIGMA interim (total 4.6/spread 4.2/ml 4.2); LEAGUE_AVG_TOTAL 222→229; copula approx ×0.87; G_OUTS_UNDER WP block removed; HITS over→shadow; R11 reclassified; value_parlay +EV gate; MLB_PARK_FACTORS stale warning. DEFERRED/DATA_GATED:
  ...
  - **CLV capture NRFI/YRFI/TEAM_TOTAL** (§EE): **SHIPPED 2026-06-09** — TEAM_TOTAL via `team_totals` team-filtered matcher, NRFI/YRFI via `totals_1st_1_innings`; also mapped PA/PR/RA/PRA combos + latent ER/BB/RBI/RUNS/GOALS/NHLPTS/NHLBLK/SV, fixed prop-log F5 keys (were full-game → never matched), added `_is_capturable_stat` guard (unmapped stats skip+warn instead of holding daemons open). SKIP_STATS now {GOLF_WIN, PARLAY, GA, PC}.
  ...
  - **REC Poisson→NB** at NFL go-live (n≥50). **HRR r=1.5→~1.1 (within-player starters; 1.83 was wrong direction)** — July refit: fix r, audit μ projection path, investigate zero-inflated NB, reset shadow log post-fix. **game-line edge floor** ~0.03–0.05 (n≥50 graded).
- **SGP Platt calibration gate**: need 100 scored SGP slips — **69/100** as of 2026-06-12. Current Platt (A=1.4988, B=−0.8102) built on NBA props; applying to SGP leg probs over-corrects (model→58% vs 69% actual win rate). Gate: 100 scored SGP slips before any Platt refit on SGP data.
...
- **HRR go-live gate — evaluated 2026-06-09, NOT activated**: n=61, 47.5% WR vs 55.0% breakeven, −21.1% sized ROI, overconfident −13.9pp. `G_HRR_OVER_LOW_LINE` added: blocks HRR over at line ≤0.5 (−25.5% ROI segment, n=54). HRR under + over at lines >0.5 stay in SHADOW_STATS. Implied r≈1.1 (not 1.5; the ~1.83 expectation was wrong direction). G13B win_prob thresholds still active (≥0.58 at line ≤0.5, ≥0.65 at >0.5). Next: July refit per Plan 10 entry above.
...
- **WNBA go-live gate: CLOSED 2026-06-09** at 98/100 (user-approved shortfall). Gate removed from gate_check.py. Early-season factor recalibration (0.80/0.90) **deferred to 2027 season start** (DATA_GATED, Plan 6 §14). REB mult pinned to 0.10 floor (35.3% shadow WR), revisit at n≥50.
...
- **SB26-131 ops note (CO, URGENT BEFORE AUG 12 2026)**: credit-card deposit ban + max 6 deposits/24h effective 2026-08-12. Action: switch to ACH as primary funding; cache working balance at each book. Not a code change (Plan 9 §9D).
...
- **`_POS_FLOW` PG receiver fix** (2026-05-10): NBA API never returns position=PG → PG receiver slot was always skipped (SG injuries redistributed only 78% of minutes). PG weight folded into SG; same-position weights unchanged (empirical flow analysis inconclusive; intuitive weights used).
...
| File | Purpose |
|------|---------|
| `engine/run_picks.py` | Main betting engine orchestrator (**~1568 lines**; refactored 2026-06-12 from 6982 — constants + logic extracted into 30+ focused modules, see **Engine Module Map** below). `main()` is decomposed into named stage helpers. **Source of truth — edit engine/ only. Root entry points are shims — no sync step needed.** Flags: `--force-card` (override card guard), `--no-cache` (bypass 15-min Odds API cache — picks pipeline only). |
