...
- **Plan 10 implementation (2026-06-07, commits 1dfb31f/13fb3f3 + EdgeModel f084010)**: SHIPPED — tier moves (RBI/ER→T1, RUNS→T1B, GA/SV→T3, HA→T1, REC→T2); MLB GAME_SIGMA interim (total 4.6/spread 4.2/ml 4.2); LEAGUE_AVG_TOTAL 222→229 (EdgeModel); copula approx ×0.87; G_OUTS_UNDER WP block removed; HITS over→shadow; R11 reclassified+blocked-log; value_parlay +EV gate; MLB_PARK_FACTORS stale warning. DEFERRED/DATA_GATED below.
  - **TEAM_TOTAL/ML_DOG not BM-shrunk (Plan 10 §1b/1c premise void)**: `apply_bm_shrinkage` has one call site (run_picks.py prop pipeline); game-line stats (TEAM_TOTAL, ML_DOG) come from `evaluate_game_lines` and never reach BM. The "double-shrink" premise does not hold against current code — NO code change made. If BM is ever extended to game lines, revisit the TEAM_TOTAL w=0.95 / ML_DOG exclusion ideas then.
  - **CLV capture for NRFI/YRFI/TEAM_TOTAL (Plan 10 §EE, 2d DEFERRED)**: removing them from capture_clv.SKIP_STATS alone captures nothing — none have market mappings (TEAM_TOTAL needs a `team_totals` team-matching branch like ML; NRFI/YRFI need confirmed Odds API first-inning market keys). Follow-up: add the capture plumbing, then un-skip. SKIP_STATS unchanged for now.
  - **BM_SHRINKAGE_WEIGHT direction (§B)**: T1 has worst ROI (−0.10) yet w=0.75 > T3 w=0.70 — inverted. At n≥150 graded/family, refit w per OOS Brier (T1 w should drop to ≤0.70–0.72). Formula is a linear opinion pool (Stone 1961), not Baker–McHale — relabel comment when refit. Update BM_SHRINKAGE_WEIGHT then.
  - **conf early-season double-shrink (§V)**: conf=0.70/0.85 shrinks toward 0.50 while BM also shrinks toward market — double-count. Replace with a GP-conditioned BM weight `w_eff = w·GP/(GP+k)`. DATA_GATED on early-season calibration backtest.
  - **VAKE_MULT['variance'] double-count (§K)**: double-counts with BM shrinkage; retire into the Kelly-stack consolidation (DATA_GATED, n≥50/market).
  - **KELLY_MARKET_MULT uncalibrated (§K)**: bias patches (PTS over=0.50 on the best-ROI family) mislocated in the sizing layer. DATA_GATED: calibrate at n≥50/market.
  - **PRA/PR/PA/RA→T1B (§A)**: positive component ρ inflates variance-of-sum. Move PR/PRA/RA to T1B at the combo Platt gate (n=100); PA may stay T2.
  - **CLV write-gate latch (§EE)**: T-10 first-poll latch can precede final injury bulletins (T-15→0). Change to last pre-tip observation (latch latest, commit ~T-3..T-5). Needs careful implementation; research-gated.
  - **MLB_PARK_FACTORS stale (§M)**: docstring warning added 2026-06-07; values NOT yet refit (TEX inverted, COL low, KC/MIN/DET hitter-friendly). Refit from current Fangraphs/Savant before applying.
  - **REC Poisson→NB (§A)**: NB candidate at NFL go-live (n≥50). **HRR NB r=1.5→~1.83 (§A)**: moment-match stale; refit at MLB batter gate. **game-line edge floor (§H)**: add a lower floor ~0.03–0.05 for game lines; DATA_GATED on graded game-line ROI by edge bucket (n≥50).
