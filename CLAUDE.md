...
- `NB_R_WNBA` (run_picks.py): `AST`=11.37, `REB`=10.74. Calibrated 2026-06-04 from 202 players / 13,322 WNBA game-logs (2023–2026 RS, min≥8, n≥10 per player). Var/mu≈1.21/1.40 confirms NB over Normal for these stats.
...
- `SIGMA_WNBA` (run_picks.py): `PTS`=mult 0.618/min 3.5; `AST`=mult 0.779/min 1.0; `REB`=mult 0.633/min 1.0; `3PM`=mult 0.48/min 0.70 (Normal path; NB_R not used for WNBA 3PM). Refit 2026-06-04 from 202 players / 13,322 games (2023–2026 WNBA RS). Used for z-score proxy and combo sigma calculations.
