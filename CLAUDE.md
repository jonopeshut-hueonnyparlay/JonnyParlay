...
- **Plan 10 fixes shipped** (commits 1dfb31f/13fb3f3/f084010, 2026-06-07): tier moves (RBI/ER→T1, RUNS→T1B, GA/SV→T3, HA→T1, REC→T2); MLB GAME_SIGMA interim (total 4.6/spread 4.2/ml 4.2); LEAGUE_AVG_TOTAL 222→229; copula approx ×0.87; G_OUTS_UNDER WP block removed; HITS over→shadow; R11 reclassified; value_parlay +EV gate; MLB_PARK_FACTORS stale warning. DEFERRED/DATA_GATED:
  ...
  - **CLV capture NRFI/YRFI/TEAM_TOTAL** (§EE): **SHIPPED 2026-06-09** (CLV-gap audit) — TEAM_TOTAL via `team_totals` team-filtered matcher, NRFI/YRFI via `totals_1st_1_innings`; same commit also mapped PA/PR/RA/PRA combos + latent ER/BB/RBI/RUNS/GOALS/NHLPTS/NHLBLK/SV, fixed prop-log F5 keys (were full-game → never matched; F5_ML would capture WRONG line), and added `_is_capturable_stat` guard (unmapped stats skip+warn instead of queueing — they were holding daemons open 21h+ and leaving blank rows). SKIP_STATS now {GOLF_WIN, PARLAY, GA, PC}.
...
- **HRR go-live gate — evaluated 2026-06-09, NOT activated**: gate met at n=61 (29W/32L, 47.5% WR vs 55.0% breakeven, −21.1% sized ROI, model overconfident −13.9pp, all calibration bins ≥0.55 overconfident, confidence anti-predictive). `G_HRR_OVER_LOW_LINE` added: blocks HRR over at line ≤0.5 (the −25.5% ROI / 46.3% WR segment, n=54). HRR under + over at lines >0.5 remain in SHADOW_STATS accumulating. Within-player starters implied r≈1.1 (not 1.5; documented ~1.83 expectation was wrong direction). G13B win_prob thresholds still active (≥0.58 at line ≤0.5, ≥0.65 at line >0.5). Next: July refit per the Plan 10 DATA_GATED entry above.
