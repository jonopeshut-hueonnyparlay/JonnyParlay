...
- `NB_R` (`calibrated.py`): `3PM`=9.15 (var/mu=1.1486); `AST`=9.66 (var/mu=1.323, P1.3 bias-corrected Jensen-MoM from EdgeModel producer 2026-06-16; was 12.16 — inflating formula); `REB`=13.16 (var/mu=1.387, P1.3 bias-corrected Jensen-MoM 2026-06-16; was 14.7); `HA`=13.41 (var/mu=1.204); `RBI`=0.87 (var/mu=1.535, ~74% zero-RBI games); `ER`=4.75 (var/mu=1.509, Task#1 starts-only re-align 2026-06-16, n=370 pitchers/15,297 starts; was 2.62 relief-contaminated); `HRR`=1.5 (moment-matched shadow log); `TB`=1.3 (var/mu=2.117, fallback — calc_tb_prob uses component Poisson convolution when TB_1B available). STL/BLK/TOV Poisson confirmed (var/mu=1.072/1.113/1.050).
...
- **SGP copula: t-copula (ν=6), `copula_joint_prob` in `quant/copula.py` (2026-06-18)**: replaced the Gaussian copula (zero tail dependence) for the final-slip joint prob, joint-EV gate, premium sizing, and embed display — NBA + MLB. `n_samples` 300→1000 (NBA ranking) / 4000→10,000 (final default). `COPULA_DF=6` (Demarta & McNeil 2005). The 91k-combo search (`copula_joint_approx`) stays Gaussian — ordinal ranking only. **Stale follow-up (DATA_GATED, not now):** `copula_joint_approx`'s `×0.87` deflator (Plan 10 §Z) is still fit vs Gaussian MC → ~1–3pp too aggressive vs the t-copula truth; re-fit vs t-copula MC at the next SGP recalibration (n=100 SGP-Platt slip gate / n≥160 MLB ρ refit). Impact negligible until then (weight-0.30, normalized, clamped).
